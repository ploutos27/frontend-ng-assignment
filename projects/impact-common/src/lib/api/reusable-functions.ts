...
export const findMostFrequest = (arr) => {
  return [...new Set(arr.map((item) => item.from))]; //ne Set let us store unique values
}
  ...
  let notifications = localStorage.getItem('users-notifications');
   if (notifications != null) {
     let usersNotifications = JSON.parse(notifications),
       sendReceivedEachUser = [],
       selectedUser = usersNotifications.find((x) => x.email == email);
      for (const u of arr) {
         sendReceivedEachUser.push({
          email: usersNotifications.find((x) => x.email === u).email,
          send: usersNotifications.find((x) => x.email === u).notifications.filter(j => j.from === email).length,
